...
for medium = 1 : NMedia
    ...
    for I = 1 : N( medium )
        ...
        [ AD0, AE0, AD1, AE1, AD2, AE2 ] = contrib( Node, x, rkT, V, rhoElt, c2RElt, c2IElt, ...
            hElt, deltat, alpha, beta, AD0, AE0, AD1, AE1, AD2, AE2 );
...
n = length( AE0 );
A0 = spdiags( [ AE0 AD0 AE0 ], -1:1, n, n );
A1 = spdiags( [ AE1 AD1 AE1 ], -1:1, n, n );
...
for Itime = 1 : MaxIT
    ...
    [ U0, U1 ] = step( A0, A1, AD2, AE2, U0, U1, time, rkT, omega, Bdry, WS, Isd, Pulse, alpha, beta, V, deltat2 );
    [ Green, RTSrd, RTSrr, Itout ] = extract( U0, U1, Ik, time, Itout, rkT, deltak, rr, Pos, Z, WR, Ird, Green, RTSrd, RTSrr, tout, deltat, Ntout, Bdry, V ); % Extract soln for desired receivers
    if ( Itout > Ntout )
        return;
    end
end
