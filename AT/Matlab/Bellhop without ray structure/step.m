function [ x2, Tray2, p2, q2, tau2, Rfa2, c2 ] = ...
    step(  x0, Tray0, p0, q0, tau0, Rfa0, c0, ...
...
[ c0, gradc0, crr0, crz0, czz0, Layer ] = ssp( x0, zSSPV, cSSPV, czV, TopOpt );
csq0 = c0 .* c0;
...
CrossIA = uint16( find( x( :, 2) < zSSPV( Layer( : ) ) ) );
if ( isempty( CrossIA ) == 0 )
  h( CrossIA ) = ( zSSPV( Layer(  CrossIA ) ) - x0( CrossIA, 2) ) ./ ...
                 ( Tray0( CrossIA, 2) .* c0( CrossIA ) );
end

Layer1 = double( Layer ); % double variable needed to do arithmetic in next line
Layer1 = Layer1 + 1;
CrossIB = uint16( find( x( :, 2) > zSSPV( Layer1 ) ) );
if ( isempty( CrossIB ) == 0 )
  h( CrossIB ) = ( zSSPV( Layer1( CrossIB ) ) - x0( CrossIB, 2) ) ./ ...
...
h = max( h, 1e-2 * deltas );   % make sure we make some motion
...
[ c1, gradc1, crr1, crz1, czz1, Layer ] = ssp( x1, zSSPV, cSSPV, czV, TopOpt );
csq1 = c1 .* c1;
cnn1_csq1 = crr1 .* Tray1(2).^2 - 2.0 * crz1 .* Tray1(1) .* Tray1(2) + czz1 .* Tray1(1).^2;

if ( isempty( CrossIA ) == 0 )
h( CrossIA ) = ( zSSPV( Layer0( CrossIA ) ) - x0( CrossIA, 2) ) ./ ...
...
[ c2, gradc2, crr2, crz2, czz2, Layer ] = ssp( x2, zSSPV, cSSPV, czV, TopOpt );
